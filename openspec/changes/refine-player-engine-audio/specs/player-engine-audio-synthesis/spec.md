...
### Requirement: Player engine audio SHALL use a sine-wave synthesis source
The race scene SHALL synthesize the player engine audio from a Web Audio oscillator configured as a sine wave, so the base timbre remains rounded and soft instead of using the existing harsher triangle-wave tone.

#### Scenario: Player race audio starts with sine waveform
- **WHEN** a player vehicle creates its engine audio chain during race scene setup
- **THEN** the oscillator source used for the player engine SHALL be configured with the `sine` waveform type
